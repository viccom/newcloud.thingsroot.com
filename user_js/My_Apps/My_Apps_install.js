...
$('.creat_templ').click(function () {
    window.open("/My_Template_creat.html?appid="+appid);
});
...
function app_detail(id) {
    ...
    $.ajax({
        ...
        success: function (data) {
            // console.log(data);
            $("span.inst_name").text(data.message.app_name);
        ...
        if(data[i].type=="section" && data[i].name=="serial_section") {
            ...
            for (var k = 0; k < child.length; k++) {
                ...
                if (child[k].type == "dropdown") {
                    app_config.serial[child[k].name] = Number($("select[name=" + child[k].name + "]").val())
                }

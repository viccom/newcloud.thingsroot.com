...
$("body").on("click", "button.update_app", function () {
    ...
    var task_desc = '升级应用'+ 'freeioe_Vserial';
    var id = 'app_upgrade/' + gate_sn + '/ '+ 'freeioe_Vserial' +'/'+ Date.parse(new Date())
    var _act = {
        "device": gate_sn,
        "data": {"inst": 'freeioe_Vserial', "name":appid},
        "id": id
    };

    gate_exec_action(app_action, _act, task_desc, 'freeioe_Vserial', app_action, "1");
